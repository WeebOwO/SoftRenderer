...
// = a + (b - a) * t
template <size_t N, typename T>
inline Vector<N, T> VectorLerp(const Vector<N, T> &a, const Vector<N, T> &b,
                               float t) {
  return a + (b - a) * t;
}
...
template <size_t N, typename T>
inline Vector<N, T> VectorBetween(const Vector<N, T> &minx,
...
inline Vector<COL, T> operator*(const Vector<ROW, T> &a,
  ...
  Vector<COL, T> b;
  for (size_t i = 0; i < COL; i++) b[i] = VectorDot(a, m.Col(i));
  return b;
}
...
inline Vector<ROW, T> operator*(const Matrix<ROW, COL, T> &m,
  ...
  Vector<ROW, T> b;
  for (size_t i = 0; i < ROW; i++) b[i] = VectorDot(a, m.Row(i));
  return b;
}
...
inline static Mat4x4f MatrixSetLookat(const Vec3f &eye, const Vec3f &at,
                                      const Vec3f &up) {
...
inline static Mat4x4f MatrixSetPerspective(float fovy, float aspect, float zn,
                                           float zf) {
