...
class ShaderContext {
 private:
  Mat4x4f m_model_mat, m_view_mat, m_projection_mat, m_normal_mat,
      m_viewport_mat;
  Light m_light;
...
inline Mat4x4f ShaderContext::GetTransformMatrix() {
  return m_model_mat * m_view_mat * m_projection_mat;
}
...
inline void ShaderContext::SetModelMatrix(const Mat4x4f &model) {
  m_model_mat = model;
}

inline void ShaderContext::SetViewMatrix(const Mat4x4f &view) {
  m_view_mat = view;
}

inline void ShaderContext::SetProjectionMatrix(const Mat4x4f &projection) {
  m_projection_mat = projection;
}

inline void ShaderContext::SetNormalMatrix(const Mat4x4f &normal) {
  m_normal_mat = normal;
}

inline void ShaderContext::SetLight(const Light &light) { m_light = light; }

inline void ShaderContext::SetViewPortMatrix(const Mat4x4f &viewport_mat) {
  m_viewport_mat = viewport_mat;
}

inline Shader::Shader(VertexShader vs_shader, FragmentShader fs_shader)
    : m_vertex_shader(vs_shader), m_fragment_shader(fs_shader) {}

inline void Shader::SetVertexShader(VertexShader vs_shader) {
  m_vertex_shader = vs_shader;
}

inline void Shader::SetFragmentShader(FragmentShader fs_shader) {
  m_fragment_shader = fs_shader;
}
