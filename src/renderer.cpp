...
namespace utils {
void BarycentricInterplate(ShaderVaryingData &output,
                           const std::array<ShaderVaryingData, 3> &vertex_array,
                           const Vec3f &barycentric) {
  Vec3f sreen_coord_a = vertex_array[0].scrren_coord,
        sreen_coord_b = vertex_array[1].scrren_coord,
        sreen_coord_c = vertex_array[2].scrren_coord;
  float z = 1 / (barycentric.x * (1 / sreen_coord_a.z) +
                 barycentric.y * (1 / sreen_coord_b.z) +
                 barycentric.z * (1 / sreen_coord_c.z));
  float a_trans_inverse_z = 1 / sreen_coord_a.z,
        b_trans_inverse_z = 1 / sreen_coord_b.z,
        c_trans_inverse_z = 1 / sreen_coord_c.z;
  float a_part = a_trans_inverse_z * barycentric.x,
        b_part = b_trans_inverse_z * barycentric.y,
        c_part = c_trans_inverse_z * barycentric.z;
  // 颜色属性插值
  float r =
      z * (a_part * vertex_array[0].color.r + b_part * vertex_array[1].color.r +
           c_part * vertex_array[2].color.r);
  float g =
      z * (a_part * vertex_array[0].color.g + b_part * vertex_array[1].color.g +
           c_part * vertex_array[2].color.g);
  float b =
      z * (a_part * vertex_array[0].color.b + b_part * vertex_array[1].color.b +
           c_part * vertex_array[2].color.b);
  output.color = Vec3f(r, g, b);
  // 世界坐标插值
  float world_x = z * (a_part * vertex_array[0].world_pos.x +
                       b_part * vertex_array[1].world_pos.x +
                       c_part * vertex_array[2].world_pos.x);
  float world_y = z * (a_part * vertex_array[0].world_pos.y +
                       b_part * vertex_array[1].world_pos.y +
                       c_part * vertex_array[2].world_pos.y);
  float world_z = z * (a_part * vertex_array[0].world_pos.z +
                       b_part * vertex_array[1].world_pos.z +
                       c_part * vertex_array[2].world_pos.z);
  output.world_pos = Vec3f(world_x, world_y, world_z);
  // 法线坐标插值
  float normal_x = z * (a_part * vertex_array[0].normal.x +
                        b_part * vertex_array[1].normal.x +
                        c_part * vertex_array[2].normal.x);
  float normal_y = z * (a_part * vertex_array[0].normal.y +
                        b_part * vertex_array[1].normal.y +
                        c_part * vertex_array[2].normal.y);
  float normal_z = z * (a_part * vertex_array[0].normal.z +
                        b_part * vertex_array[1].normal.z +
                        c_part * vertex_array[2].normal.z);
...
void Renderer::TickRenderer(float delta_time, const Buffer &buffer) {
  ...
  Mat4x4f projection =
      MatrixSetPerspective(3.1415926f * 0.35f, 1.0f, 1.0f, 10.0f);
  ...
  for (auto &&index : index_buffer) {
    DrawPrimitive(vertex_buffer[index[0]], vertex_buffer[index[1]],
                  vertex_buffer[index[2]]);
  }
...
void Renderer::DrawPrimitive(const Vertex &a, const Vertex &b,
                             const Vertex &c) {
  ShaderVaryingData a_trans = m_shader->RunVertexShader(a),
                    b_trans = m_shader->RunVertexShader(b),
                    c_trans = m_shader->RunVertexShader(c);
  // 齐次坐标裁剪,超出范围的话直接舍弃整个三角形
  std::array<ShaderVaryingData, 3> varying_data_array{a_trans, b_trans,
                                                      c_trans};
  ...
  for (auto &&data : varying_data_array) {
    ...
    data.scrren_coord = (data.homogeneous_coord *
                         global_context::shader_context->GetViewPortMatrix())
                            .xyz();
  }
  // 求出三角形的AABB包围盒
  Vec3f sreen_coord_a = varying_data_array[0].scrren_coord,
        sreen_coord_b = varying_data_array[1].scrren_coord,
        sreen_coord_c = varying_data_array[2].scrren_coord;
  int min_x = static_cast<int>(
      std::min({sreen_coord_a.x, sreen_coord_b.x, sreen_coord_c.x}));
  int min_y = static_cast<int>(
      std::min({sreen_coord_a.y, sreen_coord_b.y, sreen_coord_c.y}));
  int max_x = static_cast<int>(
      std::max({sreen_coord_a.x, sreen_coord_b.x, sreen_coord_c.x}));
  int max_y = static_cast<int>(
      std::max({sreen_coord_a.y, sreen_coord_b.y, sreen_coord_c.y}));
...
float Renderer::GetDeltaTime() {
  float delta_time;
  {
    ...
    auto time_span = duration_cast<duration<float>>(tick_time_point -
...
Renderer::Renderer(const WindowCreaterInfo &window_info, Shader *shader)
    : m_shader(shader) {
  SDL_CreateWindowAndRenderer(window_info.window_width,
                              window_info.window_height, SDL_WINDOW_RESIZABLE,
                              &m_window, &m_renderer);
  SDL_SetWindowTitle(m_window, window_info.title);
  m_depth_buffer.resize(
      window_info.window_height,
      std::vector<float>(window_info.window_height, 10000.0f));
}
