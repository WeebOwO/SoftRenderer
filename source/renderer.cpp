...
ShaderContext Renderer::BarycentricInterplate(std::array<Vertex, 3> &vertices,
    ...
    std::array<float, 3> interplate_factor{barycentric.x, barycentric.y,
                                           barycentric.z};
...
void Renderer::DrawPrimitive(std::array<VertexAttrib, 3> &vs_input) {
    ...
    int width = m_windowWidth, height = m_windowHeight_;
    int min_x, max_x, min_y, max_y;

    for (int i = 0; i < 3; ++i) {
        vertices[i].context.Clear();
        // 运行Vertex Shader
        vertices[i].pos = m_vertexShader(vs_input[i], vertices[i].context);
        ...
        if (vertices[i].pos.z < 0 || vertices[i].pos.z > w) {
            return;
        };
        if (vertices[i].pos.y < -w || vertices[i].pos.y > w) {
            return;
        };
        if (vertices[i].pos.x < -w || vertices[i].pos.x > w) {
            return;
        };
...
#pragma omp parallel for
    ...
    for (int cy = min_y; cy <= max_y; cy++) {
        ...
        std::for_each(
            std::execution::par, horizontal_range.begin(),
            horizontal_range.end(), [&](int cx) {
                Vec2f px = {(float)cx + 0.5f, (float)cy + 0.5f};
                // Edge Equation
                // 使用整数避免浮点误差，同时因为是左手系，所以符号取反
                int E01 =
                    -(cx - p0.x) * (p1.y - p0.y) + (cy - p0.y) * (p1.x - p0.x);
                int E12 =
                    -(cx - p1.x) * (p2.y - p1.y) + (cy - p1.y) * (p2.x - p1.x);
                int E20 =
                    -(cx - p2.x) * (p0.y - p2.y) + (cy - p2.y) * (p0.x - p2.x);
                ...
                // 因为透视投影最后一步会除以w
                float rhw = vertices[0].rhw * a + vertices[1].rhw * b +
                            vertices[2].rhw * c;

                // 进行深度测试
                if (rhw < m_depthBuffer[cy * m_windowWidth + cx]) return;
                m_depthBuffer[cy * m_windowWidth + cx] =
                    rhw;  // 记录 1/w 到深度缓存
                ...
                ShaderContext input =
                    BarycentricInterplate(vertices, Vec3f{c0, c1, c2});
                // 执行像素着色器
...
void Renderer::RenderPresent() {
    ...
    //  SDL_SetTextureBlendMode(m_swapTexture, SDL_BLENDMODE_BLEND);
    SDL_RenderCopy(m_renderer, m_swapTexture, nullptr, nullptr);
    SDL_RenderPresent(m_renderer);
}

Renderer::Renderer(const WindowInfo &window_info)
    : m_windowWidth(window_info.width), m_windowHeight_(window_info.height) {
    m_window = SDL_CreateWindow(window_info.title, SDL_WINDOWPOS_UNDEFINED,
                                SDL_WINDOWPOS_UNDEFINED, window_info.width,
                                window_info.height, SDL_WINDOW_RESIZABLE);
...
void Renderer::DrawPixel(int x, int y, const Vec4f &color) {
    ...
    *(m_frameBuffer + y * m_windowWidth + x) = final_color;
}
...
void Renderer::SetPixelShader(PixelShader pixel_shader) {
    m_pixelShader = std::move(pixel_shader);
}
