...
template <typename T> struct Vector<4, T> {
    ...
    explicit Vector(const T* ptr) : x(ptr[0]), y(ptr[1]), z(ptr[2]), w(ptr[3]) {}
    const T& operator[](size_t i) const {
        assert(i < 4);
        return m[i];
    }
...
// = (+a)
template <size_t N, typename T> Vector<N, T> operator+(const Vector<N, T>& a) { return a; }

// = (-a)
template <size_t N, typename T> Vector<N, T> operator-(const Vector<N, T>& a) {
    Vector<N, T> b;
    for (size_t i = 0; i < N; i++)
        b[i] = -a[i];
    return b;
...
// = (a == b) ? true : false
template <size_t N, typename T> bool operator==(const Vector<N, T>& a, const Vector<N, T>& b) {
    for (size_t i = 0; i < N; i++)
...
// = a * x
template <size_t N, typename T> Vector<N, T> operator*(const Vector<N, T>& a, T x) {
    Vector<N, T> b;
    for (size_t i = 0; i < N; i++)
        b[i] = a[i] * x;
    return b;
...
// = x * a
template <size_t N, typename T> Vector<N, T> operator*(T x, const Vector<N, T>& a) {
    Vector<N, T> b;
    for (size_t i = 0; i < N; i++)
        b[i] = a[i] * x;
    return b;
...
// a += b
template <size_t N, typename T> Vector<N, T>& operator+=(Vector<N, T>& a, const Vector<N, T>& b) {
    for (size_t i = 0; i < N; i++)
        a[i] += b[i];
    return a;
}
...
// a *= x
template <size_t N, typename T> Vector<N, T>& operator*=(Vector<N, T>& a, T x) {
    for (size_t i = 0; i < N; i++)
        a[i] *= x;
    return a;
}
...
// = |a| ^ 2
template <size_t N, typename T> T vector_length_square(const Vector<N, T>& a) {
    T sum = 0;
    for (size_t i = 0; i < N; i++)
        sum += a[i] * a[i];
    return sum;
...
// = |a|
template <size_t N, typename T> T vector_length(const Vector<N, T>& a) {
    return sqrt(vector_length_square(a));
}

// = |a| , 特化 float 类型，使用 sqrtf
template <size_t N> float vector_length(const Vector<N, float>& a) {
    return sqrtf(vector_length_square(a));
}

// = a / |a|
template <size_t N, typename T> Vector<N, T> vector_normalize(const Vector<N, T>& a) {
    return a / vector_length(a);
}

// 矢量点乘
template <size_t N, typename T> T vector_dot(const Vector<N, T>& a, const Vector<N, T>& b) {
    T sum = 0;
    for (size_t i = 0; i < N; i++)
        sum += a[i] * b[i];
    return sum;
...
// 二维矢量叉乘，得到标量
template <typename T> T vector_cross(const Vector<2, T>& a, const Vector<2, T>& b) {
    return a.x * b.y - a.y * b.x;
}

// 三维矢量叉乘，得到新矢量
template <typename T> Vector<3, T> vector_cross(const Vector<3, T>& a, const Vector<3, T>& b) {
    return Vector<3, T>(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}

// 四维矢量叉乘：前三维叉乘，后一位保留
template <typename T> Vector<4, T> vector_cross(const Vector<4, T>& a, const Vector<4, T>& b) {
...
// 输出到文本流
template <size_t N, typename T> std::ostream& operator<<(std::ostream& os, const Vector<N, T>& a) {
    os << "[";
    for (size_t i = 0; i < N; i++) {
        os << a[i];
        if (i < N - 1) os << ", ";
...
// 输出成字符串
template <size_t N, typename T> std::string vector_repr(const Vector<N, T>& a) {
    std::stringstream ss;
    ss << a;
    return ss.str();
}
...
template <size_t ROW, size_t COL, typename T>
Matrix<ROW, COL, T> operator+(const Matrix<ROW, COL, T>& a, const Matrix<ROW, COL, T>& b) {
    Matrix<ROW, COL, T> out;
    for (size_t j = 0; j < ROW; j++) {
        for (size_t i = 0; i < COL; i++)
...
Matrix<ROW, NEWCOL, T> operator*(const Matrix<ROW, COL, T>& a, const Matrix<COL, NEWCOL, T>& b) {
...
template <size_t ROW, size_t COL, typename T>
Vector<COL, T> operator*(const Vector<ROW, T>& a, const Matrix<ROW, COL, T>& m) {
    Vector<COL, T> b;
    for (size_t i = 0; i < COL; i++)
...
template <size_t ROW, size_t COL, typename T>
Vector<ROW, T> operator*(const Matrix<ROW, COL, T>& m, const Vector<COL, T>& a) {
    Vector<ROW, T> b;
    for (size_t i = 0; i < ROW; i++)
...
// 行列式求值：一阶
template <typename T> T matrix_det(const Matrix<1, 1, T>& m) { return m[0][0]; }

// 行列式求值：二阶
template <typename T> T matrix_det(const Matrix<2, 2, T>& m) {
    return m[0][0] * m[1][1] - m[0][1] * m[1][0];
}

// 行列式求值：多阶行列式，即第一行同他们的余子式相乘求和
template <size_t N, typename T> T matrix_det(const Matrix<N, N, T>& m) {
    T sum = 0;
    for (size_t i = 0; i < N; i++)
...
// 余子式：一阶
template <typename T> T matrix_cofactor(const Matrix<1, 1, T>& m, size_t row, size_t col) {
    return 0;
}
...
// 伴随矩阵：即余子式矩阵的转置
template <size_t N, typename T> Matrix<N, N, T> matrix_adjoint(const Matrix<N, N, T>& m) {
    Matrix<N, N, T> ret;
...
template <size_t N, typename T> Matrix<N, N, T> matrix_invert(const Matrix<N, N, T>& m) {
...
template <typename T> bool NearEqual(T x, T y, T error) { return (Abs(x - y) < error); }

template <typename T> T Between(T xmin, T xmax, T x) { return Min(Max(xmin, x), xmax); }

// 截取 [0, 1] 的范围
template <typename T> T Saturate(T x) { return Between<T>(0, 1, x); }
...
// 矢量转换整数颜色
static uint32_t vector_to_color(const Vec3f& color) { return vector_to_color(color.xyz1()); }
...
static Mat4x4f matrix_set_lookat(const Vec3f& eye, const Vec3f& at, const Vec3f& up) {
...
static Mat4x4f matrix_set_perspective(float fovy, float aspect, float zn, float zf) {
